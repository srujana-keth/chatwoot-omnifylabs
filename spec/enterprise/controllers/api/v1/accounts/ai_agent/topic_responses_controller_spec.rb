...
RSpec.describe 'Api::V1::Accounts::AiAgent::TopicResponses', type: :request do
  ...
  let(:another_topic) { create(:ai_agent_topic, account: account) }
  let(:another_document) { create(:ai_agent_document, account: account, topic: topic) }
  ...
  describe 'GET /api/v1/accounts/:account_id/ai_agent/topic_responses' do
    context 'when no filters are applied' do
      before do
        create_list(:ai_agent_topic_response, 30,
                    account: account,
                    topic: topic,
    ...
    context 'when filtering by topic_id' do
      before do
        create_list(:ai_agent_topic_response, 3,
                    account: account,
                    topic: topic,
                    documentable: document)
        create_list(:ai_agent_topic_response, 2,
                    account: account,
                    topic: another_topic,
                    documentable: document)
      end
  ...
  describe 'GET /api/v1/accounts/:account_id/ai_agent/topic_responses/:id' do
    let!(:response_record) { create(:ai_agent_topic_response, topic: topic, account: account) }
  ...
  describe 'PATCH /api/v1/accounts/:account_id/ai_agent/topic_responses/:id' do
    let!(:response_record) { create(:ai_agent_topic_response, topic: topic) }
