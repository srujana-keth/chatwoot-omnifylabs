...
describe Enterprise::Billing::HandleStripeEventService do
  ...
  describe 'subscription update handling' do
    ...
    it 'resets ai_agent usage on subscription update' do
      # Prime the account with some usage
      5.times { account.increment_response_usage }
      expect(account.custom_attributes['ai_agent_responses_usage']).to eq(5)
      ...
      expect(account.reload.custom_attributes['ai_agent_responses_usage']).to eq(0)
    end
  end
