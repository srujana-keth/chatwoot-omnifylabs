...
RSpec.describe AiAgent::Copilot::ChatService do
  ...
  let(:mock_ai_agent_agent) { instance_double(AiAgent::Agent) }
  let(:mock_ai_agent_tool) { instance_double(AiAgent::Tool) }
  ...
  describe '#generate_response' do
    before do
      ...
      allow(AiAgent::Agent).to receive(:new).and_return(mock_ai_agent_agent)
      allow(mock_ai_agent_agent).to receive(:execute).and_return(true)
      allow(mock_ai_agent_agent).to receive(:register_tool).and_return(true)

      allow(AiAgent::Tool).to receive(:new).and_return(mock_ai_agent_tool)
      allow(mock_ai_agent_tool).to receive(:register_method).and_return(true)
