class Api::V1::Accounts::AiAgent::TopicResponsesController < Api::V1::Accounts::BaseController
  ...
  def create
    @response = Current.account.ai_agent_topic_responses.new(response_params)
  ...
  def set_topic
    @topic = Current.account.ai_agent_topics.find_by(id: params[:topic_id])
  end

  def set_responses
    @responses = Current.account.ai_agent_topic_responses.includes(:topic, :documentable).ordered
  end
